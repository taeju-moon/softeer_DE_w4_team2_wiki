...
`docker-compose`로 Spark Standalone 클러스터(master + worker 2대)와 Jupyter(pandas/geopandas/scikit-learn) 컨테이너를 함께 띄웁니다. 무거운 집계는 Spark 배치 스크립트(`jobs/`)로, 지리 정보 처리·시각화·최종 분석은 Jupyter 노트북(`notebooks/`)에서 수행합니다.
...
- `nyc_bus_route.py` (pandas/geopandas) : GTFS 버스 노선 데이터에서 JFK 통과 노선의 Zone 커버리지·배차간격 JSON 생성
...
| `output/bus_data/route_zone_coverage.json` | JFK 통과 버스 노선별 커버 Zone 목록 및 배차간격(peak/offpeak/시간대별) |
...
- UN/Turtle Bay South, Yorkville East 등 나머지 상위 후보들도 버스 회사가 배차 증편/신설을 검토할 수 있는 2차 우선순위 노선으로 함께 제안한다.
